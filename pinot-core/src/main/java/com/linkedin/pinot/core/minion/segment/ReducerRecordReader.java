...
public class ReducerRecordReader implements RecordReader {
  ...
  public ReducerRecordReader(File indexDir, RecordAggregator recordAggregator, List<String> aggregationColumns)
      throws Exception {
    _recordReader = new PinotSegmentRecordReader(indexDir, null, aggregationColumns);
  ...
  public boolean hasNext() {
    ...
    while(_recordReader.hasNext()) {
      ...
      // Grouping rows by the given aggregation columns
      if (_rowGroup.isEmpty() || haveSameAggregationColumns(_rowGroup.iterator().next(), currentRow)) {
   ...
   */
  private boolean haveSameAggregationColumns(GenericRow row1, GenericRow row2) {
    for (String columnName : _aggregationColumns) {
