...
public class SegmentConverterTest {
  ...
  private static final String INTPUT_SEGMENT_NAME_PREFIX = "testSegment_";
  ...
  public void setUp() throws Exception {
    ...
    for (int i = 0; i < NUM_SEGMENTS; i++) {
      String segmentName = INTPUT_SEGMENT_NAME_PREFIX + i;
  ...
  public void testSegmentConcatenate() throws Exception {
    SegmentConverter segmentConverter = new SegmentConverterBuilder().setTableName(TABLE_NAME)
  ...
  public void testSegmentRollupWithTimeConversion() throws Exception {
    ...
    SegmentConverter segmentConverter = new SegmentConverterBuilder().setTableName(TABLE_NAME)
        ...
        .setAggregationColumns(Arrays.asList(new String[]{D1, D2, T}))
    ...
    for (int i = 0; i < outputRows.size(); i++) {
      Assert.assertEquals(outputRows.get(i).getValue(D1), expectedValue);
      Assert.assertEquals(outputRows.get(i).getValue(D2), Integer.toString(expectedValue));
      Assert.assertEquals(outputRows.get(i).getValue(M1), expectedValue * NUM_SEGMENTS * REPEAT_ROWS);
      expectedValue++;
    }
  }
  ...
  public void testMultipleOutput() throws Exception {
    ...
    for (int i = 0; i < result.size(); i++) {
      try (PinotSegmentRecordReader pinotSegmentRecordReader = new PinotSegmentRecordReader(result.get(i))) {
